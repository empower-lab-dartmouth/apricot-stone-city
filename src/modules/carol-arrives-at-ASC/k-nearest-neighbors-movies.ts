...
export const calculateDistanceBetweenTwoPoints: (
    point1: DataPoint, point2: DataPoint) => number = (
      point1, point2,
    ) => Math.sqrt(((point1.action - point2.action) ** 2)
...
/**
   ...
   */
export const findKNearestNeighbors: (movies: Movie[],
    target: Target, k: number) => Movie[] = (movies, target, k) => {
      movies.forEach((movie) => {
        movie.embedding.distance = calculateDistanceBetweenTwoPoints(movie.embedding, target);
      });
      movies = movies.sort((movie1, movie2) => movie1.embedding.distance as number
        - (movie2.embedding.distance as number));
      return movies.slice(0, k);
    };

export const getMovieNames: (movies:
    Movie[]) => string[] = (movies) => movies.map((movie) => movie.name);

export const averageMultipleMovieScores: (movies: Movie[]) => DataPoint = (movies) => {
  ...
  movies.forEach((movie) => {
    sumOfScores = {
      scienceFiction: sumOfScores.scienceFiction + movie.embedding.scienceFiction,
  ...
  const averageScores = {
    scienceFiction: sumOfScores.scienceFiction / numberOfMoviesBeingAveraged,
...
export const kSimilarFilms = (movie: Movie, k: number) => findKNearestNeighbors(
  MOVIE_LIBRARY, movie.embedding, k + 1,
).slice(1, undefined)
  .map((m) => m.name)
  .join(',\n\n');

export const infoAboutTheMovie = (movie: Movie) => `Action: ${movie.embedding.action}\n
