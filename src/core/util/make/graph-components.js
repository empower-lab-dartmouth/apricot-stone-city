...
function module(content) {
    var reducer = function (type) { return function (acc, curr) {
        ...
        if (acc[curr.id] !== undefined) {
            throw new Error("Cannot construct module with id ".concat(content.id, ", the ").concat(type, " with id ").concat(curr.id, " has a duplicate id in this module's definition."));
        }
