...
import { choice, string } from 'fp-ts';
  ...
  switch (action.type) {
    case 'start-convo-segment':
      ...
      logEventToRemote({
        ...
        choices: formatConvoChoice(convoSegment.choices, stateManager.getState()),
        ...
        contentSentToUser: formatContentSentToUser(convoSegment.convoNodes,
...
const getOrInitStateManager: (
  ...
  const {
    userId, cache, module, initialState, historyManager,
  } = params;
...
export const convoManagerConstructor: ConvoManagerConstructor = (
  ...
  return {
    respondToUserInput: async (userId, userInput, chatRenderFunctions) => {
      ...
      if (selectedUserChoice !== undefined) {
      ...
      } else if (currentConvoSegment.defaultChoice !== undefined) {
        log.debug(
        ...
        const defaultResponse = `I don't recognize your response of <i>${userInput}</i> right now. I'll remember this as a suggestion for updating choices later though :). Until then, try responding with one of the buttons in the chat keyboard.`;
        logEventToRemote({
          type: 'unrecognized-response',
          startingPath: stateManager.getCurrentConvoSegmentPath(),
          userId: stateManager.getState().userId as string,
          choices: formatConvoChoice(currentConvoSegment.choices, stateManager.getState()),
          userStateBeforeLogic: stateManager.getState(),
          contentReceivedFromUser: stateManager.getState().lastTextMessage,
          timestamp: new Date(),
          username: stateManager.getState().username as string,
